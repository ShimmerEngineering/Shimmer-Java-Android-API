...
public class ByteCommunicationSimulatorS3LNS0_16_11_w_sync_master extends ByteCommunicationSimulatorS3{
	...
	protected void txInfoMem(byte[] buffer) {
		...
		else if (buffer[1]==(byte)0x80 && buffer[2]==(byte)0x80 && buffer[3]==(byte)0x00) //0x8E 0x80 0x80 0x00
		{
			...
			mBuffer.add((byte) 0x8D);
			mBuffer.add((byte) 0x80);
			byte[] bytes = UtilShimmer.hexStringToByteArray("");
			for (byte b:bytes) {
				mBuffer.add(b);
			}

			mBuffer.add((byte) 0xA3);
