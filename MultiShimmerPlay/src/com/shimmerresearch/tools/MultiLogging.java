...
public class MultiLogging {
	...
	// takes in an object cluster array
	public MultiLogging(String myName) {
		mFileName = myName;
		File root = Environment.getExternalStorageDirectory();
		outputFile = new File(root, mFileName + ".dat");
	}

	public MultiLogging(String myName, String delimiter) {
		mFileName = myName;
	...
	/**
	 * This function takes an object cluster and logs all the data within it.
	 * User should note that the function will write over prior files with the
	 * same name.
	 * 
	 * @param objectClusterLog
	 *            data which will be written into the file
	 */
			...
			if (mFirstWrite == true) {
				...
				for (String key : m.keys()) {
					...
					if (compareStringArray(mSensorNames, key) == true) {
						for (FormatCluster formatCluster : m.get(key)) {
							...
							// Log.d("Shimmer",key + " " + mSensorFormats[p] +
							// " " + mSensorUnits[p]);
							p++;
						}
				...
				for (int k = 0; k < mSensorUnits.length; k++) {
					if (mSensorUnits[k] == "u8") {
						...
						writer.write("");
					} // center the graph, so the negative values will be
						// displayed
			...
			for (int r = 0; r < mSensorNames.length; r++) {
				...
				FormatCluster formatCluster = (FormatCluster) returnFormatCluster(
						dataFormats, mSensorFormats[r], mSensorUnits[r]); // retrieve
																			...
																			// calibrated
																			// data
				Log.d("Shimmer", "Data : " + mSensorNames[r]
						+ formatCluster.mData + " " + formatCluster.mUnits);
