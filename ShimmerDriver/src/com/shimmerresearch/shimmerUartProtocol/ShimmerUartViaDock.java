 ...
 *
 */
public class ShimmerUartViaDock extends ShimmerUart {

	public ShimmerUartViaDock(String comPort, String uniqueId){
		super(comPort, uniqueId);
	}
	...
	public String readMacId() throws ExecutionException {
		...
		byte[] rxBuf = processShimmerGetCommand(UartPacketDetails.COMPONENT_PROPERTY.MAIN_PROCESSOR.MAC, errorCode);
		...
		if(rxBuf.length >= 6) {
			...
			throw new DockException(mUniqueId, mComPort, errorCode, ErrorCodesShimmerUart.SHIMMERUART_COMM_ERR_MESSAGE_CONTENTS);
		}
	...
	public ShimmerVerObject readHwFwVersion() throws ExecutionException {
		...
		byte[] rxBuf = processShimmerGetCommand(UartPacketDetails.COMPONENT_PROPERTY.MAIN_PROCESSOR.VER, errorCode);
	...
	public long readRealWorldClockConfigTime() throws ExecutionException {
		...
		byte[] rxBuf = processShimmerGetCommand(UartPacketDetails.COMPONENT_PROPERTY.MAIN_PROCESSOR.RTC_CFG_TIME, errorCode);
	...
	public long readCurrentTime() throws ExecutionException {
		...
		byte[] rxBuf = processShimmerGetCommand(UartPacketDetails.COMPONENT_PROPERTY.MAIN_PROCESSOR.CURR_LOCAL_TIME, errorCode);
	...
	public long writeRealWorldClock(long miliseconds) throws ExecutionException {
		...
		processShimmerSetCommand(UartPacketDetails.COMPONENT_PROPERTY.MAIN_PROCESSOR.RTC_CFG_TIME, rwcTimeArray, errorCode);
	...
	public byte[] readInfoMem(int address, int size) throws ExecutionException{
		...
		byte[] rxBuf = processShimmerMemGetCommand(UartPacketDetails.COMPONENT_PROPERTY.MAIN_PROCESSOR.INFOMEM, address, size, errorCode);
	...
	public void writeInfoMem(int address, byte[] buf) throws ExecutionException{
		...
		processShimmerMemSetCommand(UartPacketDetails.COMPONENT_PROPERTY.MAIN_PROCESSOR.INFOMEM, address, buf, errorCode);
	}
